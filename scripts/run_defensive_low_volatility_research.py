...
from app.services.defensive_low_volatility import (  # noqa: E402
    PRIMARY_PARAMETERS,
    STRATEGY_ID,
    STRATEGY_IDENTITY,
    STRATEGY_VERSION,
...
AUTHORIZATION = (
    "Operator requested defensive_low_volatility@0.1.0 historical research through "
    "Minimal V1 using the existing active ten-symbol research universe."
)
...
def _markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        "# Defensive low-volatility research interpretation",
        "",
        ...
        f"Downside volatility {primary['downside_volatility_percent']:.4f}%; worst rolling "
        f"12-month return {primary['worst_rolling_12_month_return_percent']:.4f}%.",
        ...
        "or authorization to create a campaign, session, signal, order, transaction, or fill.",
    ]
    return "\n".join(lines) + "\n"
...
def _sqlite_backup() -> tuple[Path, Path]:
    ...
    backup = ROOT / "data" / "backups" / "defensive_low_volatility_pre_run.db"
...
def _archived_context(facade: MinimalV1Facade) -> dict[str, Any]:
    ...
    for run in facade.historical_runs():
        ...
        if name == "cross_sectional_momentum":
            contexts["archived_cross_sectional_momentum"] = {
                "strategy_identity": run.strategy_identity,
                "metrics": run.principal_metrics,
                "verdict": run.verdict,
            }
        elif name == "ma_crossover":
            contexts["archived_ma_crossover"] = {
                "strategy_identity": run.strategy_identity,
                "metrics": run.principal_metrics,
                "verdict": run.verdict,
            }
    if set(contexts) != {"archived_cross_sectional_momentum", "archived_ma_crossover"}:
        raise RuntimeError("Required archived research contexts are unavailable")
    return contexts
...
def main() -> None:
    ...
    try:
        ...
        run_id = f"defensive-low-volatility-{registration_id.split('-')[0]}"
